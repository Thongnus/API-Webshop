...
public class quanlyuserController {
    ...
    public ResponseEntity<?> add(@RequestBody User user){
        ...
        if(isxistUser!=null){
            ...
            return ResponseEntity.status(HttpStatus.OK).body(userservice.saveTT(user));
    ...
    public ResponseEntity<?> updateUser(@PathVariable("id") int id, @RequestBody User user) {
        ...
        if(user.getPassword()==null){
            ...
            User updatedUser = userservice.updatenopassword(existingUser);
    ...
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<?> updateUserpassword(@AuthenticationPrincipal User user, @RequestBody changepassword changepassword) {
        ...
        if(!(passwordEncoder.matches(changepassword.getCurrentPassword(),existingUser.getPassword()))){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Sai mật khẩu cũ");
        }
